...
void init_NRF24L01(void)
{
 ...
 SPI_RW_Reg(WRITE_REG + EN_AA,0x03);      //  频道0 - 5 自动 ACK应答允许 
 SPI_RW_Reg(WRITE_REG + EN_RXADDR,0x03);  //  允许接收地址频道0 - 5 
...
{	 
	...
	if(RX_DR)				// 判断是否接收到数据
	{
		...
		revale =1;			//读取数据完成标志
						...
						write_str("  Tem");
						write_data(':');
			         	write_data(((rx_buf[2]*256+rx_buf[3])/100)%10+48);
						write_data(((rx_buf[2]*256+rx_buf[3])/10)%10+48);
						write_data('.');
						write_data((rx_buf[2]*256+rx_buf[3])%10+48);
						write_data(0xdf);
						write_data('c');
						write_com(0x80+0x40);
						write_str("  Dis");
						write_data(':');
						write_data((rx_buf[0]*256+rx_buf[1])/1000%10+48);
						write_data('.');
		   				write_data((rx_buf[0]*256+rx_buf[1])/100%10+48);
		   				write_data((rx_buf[0]*256+rx_buf[1])/10%10+48);
						write_data((rx_buf[0]*256+rx_buf[1])%10+48);
						write_data('m');
	}
